...
class PlgSystemApiSef extends CMSPlugin
{
	...
	private function processApiSefUrl($segments)
	{
		...
		{
			// Skip basic routing parameters, but allow id/codigo/nome if not set from URL segments
		...
		foreach ($query as $key => $value) {
			// Skip basic routing parameters, but include id/codigo/nome if not from URL segments
			if (!in_array($key, ['option', 'app', 'resource', 'lang', 'format'])) {
				// For id/codigo, only exclude if we already set it from URL segments
				if (($key === 'id' || $key === 'codigo') && $identifier !== null) {
					continue;
		...
		// Force unlimited mode - override any configuration to always be unlimited
		$unlimitedMode = true; // Force unlimited mode
		
		if (!isset($query['limit'])) {
			if ($unlimitedMode) {
				// In unlimited mode, don't add any limit parameter (let API decide)
				file_put_contents(JPATH_ROOT . '/administrator/logs/apisef_debug.log', 
					date('Y-m-d H:i:s') . " - Unlimited mode: no limit parameter added" . PHP_EOL, FILE_APPEND);
			} else {
				// In limited mode, add custom limit
				$customLimit = $this->getCustomLimit($app);
				if ($customLimit > 0) {
					$apiUrl .= '&limit=' . $customLimit;
					file_put_contents(JPATH_ROOT . '/administrator/logs/apisef_debug.log', 
						date('Y-m-d H:i:s') . " - Limited mode: Added custom limit: $customLimit for app: $app" . PHP_EOL, FILE_APPEND);
				}
			}
		}
		...
		if (isset($query['nolimit']) && $query['nolimit'] == '1') {
			// Remove any existing limit parameter to ensure no limit
			$apiUrl = preg_replace('/&limit=\d+/', '', $apiUrl);
			file_put_contents(JPATH_ROOT . '/administrator/logs/apisef_debug.log', 
				date('Y-m-d H:i:s') . " - No limit mode activated: removed limit parameter" . PHP_EOL, FILE_APPEND);
		}
