...
class plgAPIArticlesij extends ApiPlugin
{
	public function __construct(&$subject, $config = array())
	{
		...
		//load helper file
		require_once JPATH_SITE.'/plugins/api/articlesij/articlesij/helper/simpleschema.php';

		ApiResource::addIncludePath(dirname(__FILE__).'/articlesij');

		// Load component language
		$this->loadComponentLanguage();

		// Set resources & access
		$this->setResourceAccess('articleij', 'public', 'get');   // Singular - one IJ article by ID
		$this->setResourceAccess('articlesij', 'public', 'get');  // Plural - multiple IJ articles
	...
	private function loadComponentLanguage(): void
	{
		try {
			...
			Factory::getApplication()->enqueueMessage(
				Text::sprintf('PLG_API_ARTICLESIJ_ERROR_LOADING_LANGUAGE', $e->getMessage()),
				'warning'
			);
		}
	}
