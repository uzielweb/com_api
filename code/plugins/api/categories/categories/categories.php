...
class CategoriesApiResourceCategories extends ApiResource
{
	...
	public function getCategoriesList()
	{
		...
		// Get language from URL or parameter
		$lang = $input->get('lang', '', 'string');
		if (empty($lang)) {
			// Detect language from URL path (/pt/ or /en/)
			$uri = $app->input->server->get('REQUEST_URI', '', 'string');
			if (strpos($uri, '/pt/') !== false) {
				$lang = 'pt';
		...
		$language = isset($filters['language']) ? $inputFilter->clean($filters['language'], 'string') : $languageCode;
